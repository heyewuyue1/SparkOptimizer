...
def preprocess(raw_plan):
    plan = raw_plan.replace('@', '\n')
    if 'list#' in raw_plan or 'Aggregate' in  raw_plan:
        logger.info(f'Plan contains list# or Aggregate, skipping this one...')
        return ''
    return plan  
