import os
from connectors.spark_connector_hive import find_main_select,SparkConnector
import sqlparse
import math
...
def generate_min_max_from_result(result): 
...
def rewrite_and_test_syntax(connector, query):
        ...
        try:
            ...
            logger.warning(f'Query failed: {modified_sql}, Skipping this one...')
            return

        min_max_dict = generate_min_max_from_result(res)
        logger.debug(f'Min max dict for {query}: {min_max_dict}')
