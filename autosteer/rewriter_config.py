...
from autosteer.rewrite_span import RewriteSpan
import storage
...
class RewriteExploration:
    ...
    def __init__(self, query_path):
        self.rewrite_span = RewriteSpan(query_path)
        self.query_path = query_path
        self.tunable_knobs = self.rewrite_span.get_tunable_knobs()  # the effective query optimizer knobs
    ...
    def get_measurements(self):
        ...
        stmt = '''
            SELECT walltime as total_runtime, qrc.rewrite_rules, m.time, qrc.num_rewrite_rules
            FROM queries q,
                 rewrite_measurements m,
                 query_rewrite_configs qrc
            WHERE m.query_rewrite_config_id = qrc.id
              AND qrc.query_id = q.id
    ...
    def get_baseline(self):
        ...
        runs = pdsql.sqldf('SELECT total_runtime FROM df WHERE rewrite_rules = \'None\'', locals())
        runtimes = runs['total_runtime'].to_list()
        return runtimes

    def get_promising_rewrite_measurements_by_num_rules(self, num_rewrite_rules, baseline_median, baseline_mean):
        """Get all measurements for hint-sets having a specific size"""
        measurements = self.get_measurements()
        stmt = f'''SELECT total_runtime, rewrite_rules, time
        FROM measurements
        WHERE num_rewrite_rules = {num_rewrite_rules};'''
        df = pdsql.sqldf(stmt, locals())
        measurements = df.groupby(['rewrite_rules'])['total_runtime'].agg(['median', 'mean'])
    ...
    def get_next_hint_sets(self):
        ...
        if n > len(self.tunable_knobs) or n > MAX_DP_DEPTH:
            ...
            try:
                ...
                single_optimizers = self.get_promising_rewrite_measurements_by_num_rules(1, median, mean)
                combinations_previous_run = self.get_promising_rewrite_measurements_by_num_rules(n - 1, median, mean)
    ...
    def next(self):
        ...
        return list(filter(lambda x: x in self.rewrite_span.get_tunable_knobs(), conf))
