...
"""
def get_dic(database):
    file_name = f"data/{database}/dic_column_stat.json"
    dic_column_stat = {}
    with open(file_name, "r") as file:
        dic_column_stat = json.load(file)

    file_name = f"data/{database}/dic_table_columns.json"
    dic_table_columns = {}
    with open(file_name, "r") as file:
...
def add_info(database, physical_node,num_node,lg_op):
    '''在node中添加Rowcount、sizeInBytes、hist'''
    dic_table_columns,dic_column_stat = get_dic(database)
    ...
    if op != 'Filter':  # 非filter节点，添加空的hist
        ...
        if op == 'AdaptiveSparkPlan':
            try:
                ...
                lines_n.insert(line_idx + 1, 'max_hist_len: '+ str(0))
    else: # 解析filter节点
...
def get_explain(database, timed_result_f, timed_result_c):
    ...
    physical_node = add_info(database, physical_node,num_node,lg_op)
